...
public function trier(string $variable){
    ...
    return $result = $query->fetchAll();
        ...
        if(strcmp($variable,'adresse')==0){
            $query =$pdo->prepare('SELECT * FROM livraison ORDER BY adresse DESC');
            $query->execute();
        
            return $result = $query->fetchAll();
                } 
                if(strcmp($variable,'id_commande')==0){
                    $query =$pdo->prepare('SELECT * FROM livraison ORDER BY id_commande DESC');
                    $query->execute();
                
                    return $result = $query->fetchAll();
                        }    
                        if(strcmp($variable,'id_livreur')==0){
                            $query =$pdo->prepare('SELECT * FROM livraison ORDER BY id_livreur DESC');
                            $query->execute();
