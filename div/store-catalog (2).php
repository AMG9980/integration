...
if(isset($_GET['min']) AND isset($_GET['max'])){
    ...
    $result =$c->affichertoutp((int)$_GET['min'],(int)$_GET['max']);
    $img=$c->afficherimgp((int)$_GET['min'],(int)$_GET['max']);
    if(isset($_GET['name'])){
...
elseif(isset($_GET['categorie'])){
    ...
    $result =$c->affichertoutc($_GET['categorie']);
    $img=$c->afficherimgc($_GET['categorie']);
    $catalog=$p->affichertout();
    if(isset($_GET['name'])){
        echo base64_encode($img[(int)$_GET['name']]['img']);
    exit;
    }
...
f[0]=i;
    ...
    var xhr=new XMLHttpRequest();
                ...
                xhr.open('GET','store-catalog.php?name='+i,false);
                   xhr.onload =function() {
...
produit.innerHTML+= '<div class="nk-gap-1"></div><div class="nk-product-price">'+row[i]['prix']+'dt</div><div class="nk-gap-1"></div> <a href="store-cart.php?produit='+row[i]['nom']+'&amp;prix='+row[i]['prix'] + '"class="nk-btn nk-btn-rounded nk-btn-color-dark-3 nk-btn-hover-color-main-1">Add to Cart</a>'
...
function createcategorie(){
    var result10 =  '<?php echo json_encode($catalog); ?>;'
    result10=result10.replace(";","");

    var row=JSON.parse(result10);

    var conten= document.createElement('div');
    var i=0;
    conten.className="nk-widget-categories"

    for(i=0;i<row.length;i++)
           {

            conten.innerHTML +='<li><a href="store-catalog.php?categorie='+ row[i]["id_catalog"]+'">'+row[i]["nom_catalog"]+'</a></li>'
            
        }
        document.getElementById("nk-widget-content").appendChild(conten);
...
<div class="nk-widget nk-widget-highlighted">
    ...
    <div class="nk-widget-content" id="nk-widget-content">
...
placeholder="max"
            ...
            <div>
                <div class="text-white mt-4 float-left">
                    PRICE:
                    <strong class="text-main-1">dt <span class="nk-input-slider-value-0"></span></strong>
                    -
                    <strong class="text-main-1">dt <span class="nk-input-slider-value-1"></span></strong>
                </div>
                <input type='submit' onclick="filtrer()"  class="nk-btn nk-btn-rounded nk-btn-color-white float-right">Apply</a>
            </div>
