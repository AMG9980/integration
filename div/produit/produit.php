...
public function __construct(string $produit,string $description,string $id_categorie,string $quantite1,string $prix1,string $im,string $nom)
{
...
try {
$query =$pdo->prepare('SELECT id_produit,nom,id_categorie,prix,quantite,descriptio FROM produit WHERE id_produit = ?');
...
try {
$query =$pdo->prepare('INSERT INTO produit (id_produit,descriptio,id_categorie,prix,quantite,img,nom) VALUES(?,?,?,?,?,?,?)');
...
public function modifier(){
    require 'connection.php';
    try {
    $query =$pdo->prepare('UPDATE  produit SET id_categorie= ? , prix= ? , quantite= ?  where id_produit = ? ') ;
    echo "ffff";
...
try {
$query =$pdo->prepare('SELECT nom,prix,id_categorie FROM produit');
$query->execute();
return $result = $query->fetchAll();
...
try {
$query =$pdo->prepare('SELECT nom,prix FROM produit where (prix>? AND prix<?)');
...
public function affichertoutc(string $cart){
    require 'connection.php';
    
    try {
    $query =$pdo->prepare('SELECT nom,prix FROM produit where (id_categorie=?)');
    $query->bindParam(1,$cart);
...
public function afficherimgp(){
    ...
    try {
    $query =$pdo->prepare('SELECT img FROM produit where (prix>? AND prix<?)');
  $query->bindParam(1,$min);
$query->bindParam(2,$max);  
$query->execute();
    return $result = $query->fetchAll();
    ...
    public function afficherimgc(string $cart){
    require 'connection.php';
    
    try {
    $query =$pdo->prepare('SELECT img FROM produit where (id_categorie=?)');
  $query->bindParam(1,$cart);
$query->execute();
    return $result = $query->fetchAll();
